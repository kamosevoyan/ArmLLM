...
from utils import add_postfix, remove_postfix, append_few_shot
...
def eval_dataset(model: Module, tokenizer: AutoTokenizer, dataset: DatasetDict):
    ...
    for data in tqdm(subset):
        ...
        with torch.inference_mode():
            with torch.cuda.amp.autocast(enabled=True, dtype=torch.bfloat16):
                output = model.generate(**encoded_docoment, max_new_tokens=32, do_sample=False)
...
def eval_dataset_batch(model: Module, tokenizer: AutoTokenizer, dataset: DatasetDict, batch_size: int=2):
    
    subset = dataset['validation'].shuffle().select(range(200)).map(lambda x: {'length': len(x['document']), **x}).sort('length')    
    metric = load("rouge")    
    ...
    for begin_idx in tqdm(range(0, len(subset), batch_size)):
        ...
        with torch.inference_mode():
            with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
                outputs = model.generate(**postfixed_batch_documents, max_new_tokens=32, do_sample=False)
