...
class VectorQuantizer(nn.Module):
    ...
    def diversity_loss(self, distances: Tensor):
        ...
        max = torch.log(
            torch.tensor(self.n_e, device=distances.device, dtype=distances.dtype)
        )
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch in dataloader:
            ...
            loss, _, __ = model.calculate_loss(x, x_recon, vq_loss)
            loss.backward()
            optimizer.step()
...
def visualize_results(
    model: Module, original: Tensor, reconstructed: Tensor, epoch: int
):
    ...
    with torch.no_grad():
        ...
        plt.tight_layout()
        plt.savefig(f"./logs/reconstruction_epoch_{epoch}.png")
        plt.close()
